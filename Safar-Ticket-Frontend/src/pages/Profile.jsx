...
function ChangePasswordForm() {
    ...
    return (
        <div className="mt-12 pt-8 border-t border-gray-200">
            ...
            />
            <h3 className="text-xl font-bold text-[#0D47A1] mb-6">Change Password</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
                <input className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="password" name="old_password" value={passwords.old_password} onChange={handleChange} placeholder="Current Password" required />
                <input className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="password" name="new_password" value={passwords.new_password} onChange={handleChange} placeholder="New Password" required />
                <input className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="password" name="confirm_password" value={confirmPassword} onChange={handleChange} placeholder="Confirm New Password" required />
    ...
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <header className="bg-white shadow-sm w-full">
        <nav className="container mx-auto px-6 py-4 flex items-center">
            <button onClick={() => setIsMenuOpen(true)} className="p-2 rounded-full hover:bg-gray-100 mr-4"><MenuIcon /></button>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input id="first_name" className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="text" name="first_name" value={user.first_name} onChange={handleChange} />
              </div>
              <div>
                <label htmlFor="last_name" className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                <input id="last_name" className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="text" name="last_name" value={user.last_name} onChange={handleChange} />
              </div>
            </div>
            <div>
              <label htmlFor="phone_number" className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
              <input id="phone_number" className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="tel" name="phone_number" value={user.phone_number} onChange={handleChange} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label htmlFor="birth_date" className="block text-sm font-medium text-gray-700 mb-1">Birth Date</label>
                <input id="birth_date" className="w-full px-4 py-3 border border-gray-300 rounded-lg" type="date" name="birth_date" value={user.birth_date} onChange={handleChange} />
              </div>
              <div>
                <label htmlFor="city_name" className="block text-sm font-medium text-gray-700 mb-1">City</label>
                <select id="city_name" name="city_name" value={user.city_name || ''} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white">
