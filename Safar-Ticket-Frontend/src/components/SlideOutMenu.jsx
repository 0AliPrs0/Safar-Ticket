...
function SlideOutMenu({ isOpen, onClose, user, hasPendingPayment }) {
  ...
  const menuItems = [
    { name: t('home'), icon: <HomeIcon />, path: '/' },
    { name: t('profile'), icon: <UserCircleIcon />, path: '/profile' },
    { name: t('my_bookings'), icon: <TicketIcon />, path: '/bookings', notification: hasPendingPayment },
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed top-0 start-0 h-full w-72 bg-white dark:bg-gray-800 shadow-xl z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'}`}
      >
            ...
            <nav className="flex-grow p-4 space-y-1">
                {menuItems.map(item => (
                    ...
                    >
                        ...
                        {item.notification && <span className="w-2.5 h-2.5 bg-red-500 rounded-full ms-auto"></span>}
                    </button>
                ))}
            </nav>
