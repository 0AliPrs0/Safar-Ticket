...
#             #  زمان سرور UTC است، بنابراین با زمان UTC مقایسه می‌کنیم
#             if travel_info["departure_time"] < datetime.datetime.utcnow():
#                 conn.rollback()
...
#             # --- اصلاح کوئری اصلی ---
#             # حالا فقط رزروهای پرداخت شده یا رزروهای معتبر (منقضی نشده) به عنوان اشغال شده در نظر گرفته می‌شوند
#             cursor.execute("""
...
#             reservation_time = datetime.datetime.utcnow() # استفاده از UTC برای هماهنگی
...
#                 "expires_at": expiration_time.isoformat() + "Z" # اضافه کردن Z برای مشخص کردن UTC
#             }, status=201)
...
#             # اضافه کردن 'Z' به زمان‌ها برای مشخص کردن UTC در فرانت‌اند
...
class ReserveTicketAPIView(APIView):
    def post(self, request):
        ...
        try:
            ...
            # --- اصلاح کلیدی: کنسل کردن رزروهای منقضی شده قبلی برای همین صندلی ---
            cursor.execute("""
