    ...
    def post(self, request):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE r.reservation_id = %s AND r.user_id = %s AND r.status = 'reserved' AND r.expiration_time <= NOW()
            """, (reservation_id, user_id))
            
            expired_booking = cursor.fetchone()
            if not expired_booking:
                conn.rollback()
                return Response({"error": "This reservation is not expired or does not belong to you."}, status=404)
            ...
            cursor.execute("UPDATE Reservation SET reservation_time = %s, expiration_time = %s WHERE reservation_id = %s", (new_reservation_time, new_expiration_time, reservation_id))
            conn.commit()
            ...
            try:
                ...
                if user_prefs and user_prefs.get('allow_payment_reminders') and user_email:
                    cursor.execute("""
                        SELECT dep_city.city_name AS departure_city, dest_city.city_name AS destination_city, tr.departure_time
                        FROM Travel tr
